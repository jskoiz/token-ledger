...
test("ESLint checks repository JavaScript tooling and ignores generated output", async () => {
  ...
  const toolingConfig = await eslint.calculateConfigForFile("tools/verify-release.mjs");
  ...
  assert.equal(
    await eslint.isPathIgnored(resolve(REPOSITORY_ROOT, "tools/verify-release.mjs")),
    false,
  );
});
