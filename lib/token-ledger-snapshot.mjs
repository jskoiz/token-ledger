...
async function readBoundedSnapshot(source, sourceLimit, encoding) {
  ...
  try {
    ...
    const encoded = Buffer.allocUnsafe(sourceLimit + 1);
    let bytesRead = 0;
    while (bytesRead < encoded.byteLength) {
      const result = await handle.read(
        encoded,
        bytesRead,
        encoded.byteLength - bytesRead,
        bytesRead,
      );
