...
function rememberQuotaLabel(quotaLabels, candidate) {
  if (candidate.limitName == null) return;
  const observedAt = Date.parse(candidate.timestamp);
  const current = quotaLabels.get(candidate.limitKey);
  if (
    !current ||
    observedAt > current.observedAt ||
    (
      observedAt === current.observedAt &&
      candidate.limitName.localeCompare(current.limitName) > 0
    )
  ) {
    quotaLabels.set(candidate.limitKey, {
...
function coalesceCurrentQuotaLabels(context) {
  for (const quota of context.quotas.values()) {
    const latest = context.quotaLabels.get(quota.limitKey);
    if (latest) quota.limitName = latest.limitName;
  }
}
...
async function scanRollout(path, stateRows, signal, continuityBytes = null) {
  ...
  for (const operation of result.operations) {
    ...
    if (operation.kind === "quota") {
      for (const candidate of operation.candidates) {
        rememberQuotaLabel(context.quotaLabels, candidate);
...
async function collectUsageWithConcurrency(
  ...
  try {
    ...
    coalesceCurrentQuotaLabels(context);
    context.spool.finishWrites();
...
if (!isMainThread && workerData?.tokenLedgerImporterWorker && parentPort) {
  ...
  parentPort.on("message", async ({ index, path, continuityBytes }) => {
    try {
      const result = await scanRollout(
        path,
        workerStateRows,
