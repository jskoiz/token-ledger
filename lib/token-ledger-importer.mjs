...
async function collectUsageWithConcurrency(
  ...
  try {
    ...
    const validateAndPublish = async ({ ledger }) => {
      await validateSources();
      if (!(options.publishSnapshot instanceof Function)) return;
      addDurableRowsToSpool(context, ledger);
      const candidate = {
        ...buildSnapshot(context, options, titles),
        sourceWatermark: inventory.watermark,
      };
      publishedSnapshot = await options.publishSnapshot(candidate);
    };
    const ledger = await updateDurableLedger({
      ...
      validateAfterCommit: validateAndPublish,
    });
...
async function main() {
  ...
  const snapshot = await collectUsage({
    ...
    publishSnapshot: async (candidate) => {
      ...
      writeResult = await writePrivateSnapshot(options.output, candidate);
