...
function sourceIdsForRolloutPaths(codexHome, paths) {
  ...
  for (const [baseId, group] of groups) {
    const ordered = [...group].sort((left, right) => {
      const locationOrder =
        (sourceLocationForPath(root, left) === "active" ? 0 : 1) -
        (sourceLocationForPath(root, right) === "active" ? 0 : 1);
      return locationOrder || left.localeCompare(right);
    });
    sourceIds.set(ordered[0], baseId);
    for (const path of ordered.slice(1)) {
      sourceIds.set(
        path,
        `${baseId}:copy:${hash(relative(root, path), 16)}`,
      );
    }
  }
  return sourceIds;
...
export async function sourceInventory(codexHome, includeArchived = true) {
  ...
  const rolloutSourceIds = sourceIdsForRolloutPaths(root, rolloutFiles);
  return {
    files: rolloutFiles
      .sort()
      .map((path) => ({
