...
function acceptedSourceWatermark(before, after) {
  const rolloutPaths = new Set(before.lifecycleFiles.map((entry) => entry.path));
  const entries = [
    ...before.watermarkEntries.filter(({ path }) => rolloutPaths.has(path)),
...
async function scanRollout(
  ...
  const source = bounded && maximumBytes === 0
    ? Readable.from([])
    : createReadStream(path, sourceOptions);
...
async function collectUsageWithConcurrency(
  ...
  const continuityBytes = inventory.files.map((entry) =>
    priorContinuity.get(String(entry.sourceId))?.cursorBytes ?? null
  );
  const maximumBytes = inventory.files.map((entry) => entry.size);
  ...
  try {
    await scanFiles(
      ...
      onProgress,
      (result, index) => {
        inventory.files[index].cursorFingerprint = result.cursorFingerprint;
        inventory.files[index].continuityBytes = result.continuityBytes;
        inventory.files[index].continuityFingerprint =
    ...
    if (!(await sourceInventoryPreservesCutoff(inventory, after))) {
    ...
    const validateSources = async () => {
      ...
      if (
        stagedWatermark &&
        !sourceWatermarksEqual(stagedWatermark, acceptedWatermark)
      ) {
        const error = new Error(
          "Local Codex metadata changed after the candidate snapshot was staged.",
        );
        error.code = "ERR_SOURCE_CHANGED_DURING_COLLECTION";
        throw error;
      }
      publicationWatermark = acceptedWatermark;
    };
