...
async function sourceInventoryPreservesCutoff(before, after) {
  ...
  for (const [sourceId, cutoff] of initial) {
    ...
    if (!observed || observed.size < cutoff.size) return false;
    const unchanged =
    ...
    if (
      ...
      await prefixFingerprint(observed.path, cutoff.size) !==
        cutoffFingerprint
    ) {
      return false;
    }
...
export async function sourceInventory(codexHome, includeArchived = true) {
  ...
  return {
    ...
    lifecycleFiles,
    watermarkEntries: sourceStats.map(({ path, sourceStat }) => ({
      path,
...
async function scanRollout(
  ...
  return {
    path,
    cursorFingerprint: contentHash.digest("hex"),
    continuityBytes,
...
function sourceMatchesDurableCursor(entry, prior) {
  ...
  return (
    Number(entry.size) === Number(prior.cursorBytes) &&
    Number(entry.size) === Number(prior.sizeBytes) &&
...
function sourceEnrichmentChanged(entry, prior, stateRows, titles) {
  const lastSeenAt = Date.parse(String(prior?.lastSeenAt || ""));
  if (!Number.isFinite(lastSeenAt)) return true;
  const threadId = String(entry.path).match(UUID_AT_END)?.[1];
  if (!threadId) return false;
  const stateUpdatedAt = metadataTimestampMs(stateRows.get(threadId)?.updated_at);
  const titleUpdatedAt = Number(titles.get(threadId)?.timestamp);
  return (
    (Number.isFinite(stateUpdatedAt) && stateUpdatedAt > lastSeenAt) ||
    (Number.isFinite(titleUpdatedAt) && titleUpdatedAt > lastSeenAt)
  );
}
...
function subtractCompactedRows(
  ...
  const matches = row.compactedEventKeys
    .map((eventKey) => currentRowsByEventKey.get(String(eventKey)))
    .filter(Boolean);
...
function addDurableRowsToSpool(context, ledger) {
  const currentRowsByEventKey = new Map();
...
async function collectUsageWithConcurrency(
  ...
  const priorContinuity = await readDurableSourceContinuity(
  ...
  for (let index = 0; index < inventory.files.length; index += 1) {
    ...
    if (
      ...
      !sourceEnrichmentChanged(entry, prior, state.rows, titles)
    ) {
  ...
  const maximumBytes = scanIndexes.map((index) => inventory.files[index].size);
  ...
  try {
    await scanFiles(
      ...
      (result, index) => {
        ...
        context.bytesScanned += inventory.files[inventoryIndex].size;
      },
    );
