...
function threadMetadata(threadId, stateRows, titles, parents, fallback = {}) {
  ...
  return {
    ...
    model: safeExportLabel(
      normalizeCodexCreditModel(fallback.model || row?.model || "unknown"),
      80,
      "unknown",
    ),
  ...
  function* compactableEvents() {
    for (const token of context.spool.tokenRows()) {
      ...
      const event = {
        ...
        rateCardModel: occurrence.model || metadata.model,
        ...
        rateCardCredits: calculateCodexPurchasedCredits({
...
async function scanFilesWithWorkers(
  ...
  function dispatch(worker) {
    ...
    assignments.set(worker, {
      index,
      terminalOnError: completedCount > 0,
    });
    try {
  ...
  for (const worker of workers) {
    worker.on("message", (message) => {
      ...
      if (message.error) {
        close(workerError(message.error, assignment.terminalOnError));
        return;
      }
