  ...
  return {
    files: rolloutFiles
      ...
      .map((path) => ({
        ...
        sourceId: durableSourceId(codexHome, path),
        location: path.startsWith(`${resolve(codexHome)}/archived_sessions/`)
          ? "archived"
          : "active",
      })),
...
function subtractCompactedRows(row, currentRows) {
...
function addDurableRowsToSpool(context, ledger) {
  ...
  for (const storedRow of ledger.usageRows) {
    const row = storedRow.identityKind === "compacted"
      ? subtractCompactedRows(storedRow, currentRows)
      : storedRow;
    if (!row) continue;
    ...
    if (
      row.identityKind === "migrated_compacted" ||
      row.identityKind === "compacted"
    ) {
      const calls = Math.max(0, Math.floor(row.toolCalls));
      for (let index = 0; index < calls; index += 1) {
        context.spool.insertCall(
          `${row.identityKind}-tool-${row.observationId}-${index}`,
          row.turnId,
          row.threadId,
          true,
        );
  ...
  for (const row of ledger.toolRows) {
    context.spool.insertCall(
      row.callKey,
      row.turnId,
      row.threadId,
...
function reduceScanResult(result, context) {
  const sourceId = durableSourceId(context.codexHome, result.path);
  let currentCandidate = null;
