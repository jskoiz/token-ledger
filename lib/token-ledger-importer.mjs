...
async function scanFilesSequential(files, stateRows, onProgress) {
  const results = [];
  const reportCompleted = createProgressReporter(files, onProgress);
  for (let index = 0; index < files.length; index += 1) {
    results[index] = await scanRollout(files[index], stateRows);
    reportCompleted(index);
  }
  return results;
}

async function scanFilesWithWorkers(files, stateRows, concurrency, onProgress) {
...
async function scanFiles(files, stateRows, concurrency, onProgress) {
  if (!files.length) return [];
  if (concurrency <= 1) {
    return scanFilesSequential(files, stateRows, onProgress);
  }
  return scanFilesWithWorkers(files, stateRows, concurrency, onProgress);
}
...
async function collectUsageWithConcurrency(
  ...
  try {
    const results = await scanFiles(files, state.rows, concurrency, onProgress);
    for (let index = 0; index < files.length; index += 1) {
      reduceScanResult(results[index], context);
