...
async function collectUsageWithConcurrency(
  ...
  try {
    ...
    const validateSources = async () => {
      // The staged candidate has already passed the post-scan cutoff check.
      // Source activity during the durable commit cannot change its contents,
      // so publish it with its staged watermark and let the next refresh
      // reconcile anything that arrived afterward.
      if (stagedWatermark) {
        publicationWatermark = stagedWatermark;
        return;
      }
