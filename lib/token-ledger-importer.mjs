...
async function createUsageSpool() {
  ...
  return {
    ...
    *tokenRows() {
      ...
      // Retain it in this generator until iteration has completed.
...
function reduceScanResult(result, context, inventorySourceId = null) {
  ...
  // A source-local omission can make the observed membership set incomplete.
  // Preserve prior ownership until this same source completes a clean scan.
  if (result.parseErrors > 0 || result.invalidTokenRecords > 0) {
    ...
    if (operation.kind === "quota") {
      ...
      for (const bucket of [
        ...
        addQuotaSourceBounds(
          ...
          key,
          sourceId,
          operation.occurrence.timestamp,
        );
      }
      continue;
    }
