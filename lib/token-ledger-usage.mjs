...
function tokenTotalsReconcile(totalTokens, inputTokens, outputTokens, allowFractional) {
...
export function usageBuckets(snapshot = {}) {
  return Array.isArray(snapshot.events)
    ? snapshot.events.map((row) => normalizeTokenUsage(row) ?? row)
    : [];
}
...
export function splitUsageBucketsAtBoundaries(buckets, boundaryValues) {
  ...
  for (const sourceBucket of Array.isArray(buckets) ? buckets : []) {
    const bucket = normalizeTokenUsage(sourceBucket) ?? sourceBucket;
