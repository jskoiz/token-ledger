...
export function usageCallCount(bucket) {
  ...
  return positiveSafeInteger(bucket.callCount) || 1;
}

export function usageDetailedCallCount(bucket) {
  const callCount = usageCallCount(bucket);
  const stored = positiveSafeInteger(bucket?.detailedCallCount);
  return Math.min(
    callCount,
...
export function usageInputCallCount(bucket) {
  const callCount = usageCallCount(bucket);
  const stored = positiveSafeInteger(bucket?.inputCallCount);
  return Math.min(
    callCount,
