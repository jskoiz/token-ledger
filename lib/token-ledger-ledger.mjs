...
function rowToolCallKeys(database) {
  ...
  `).all()) {
    ...
    current.push(String(row.callKey));
...
function captureUsageToolOwnership(
  ...
  const observedObservationIds = new Set();
  for (const row of tokenRows || []) {
    const eventKey = text(row?.eventKey, 4_000, "");
    const observationId = exactObservationIds.get(eventKey);
    if (observationId) observedObservationIds.add(observationId);
  }
  const deleteMembership = database.prepare(
    "DELETE FROM usage_tool_membership WHERE observation_id = ?",
  );
  for (const observationId of observedObservationIds) {
    deleteMembership.run(observationId);
  }
...
function databaseUsageRows(database, includeArchived, sources) {
  ...
  const toolCallKeys = rowToolCallKeys(database);
