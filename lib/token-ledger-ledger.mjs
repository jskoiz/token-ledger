...
async function ensureLedgerDirectory(path, privateDirectory) {
  ...
  if (privateDirectory && !existed) await chmodIfPresent(directory, 0o700);
}
...
async function openLedger(
  ...
  if (!readOnly) {
    await ensureLedgerDirectory(path, privateDirectory);
  }
...
async function acquireLedgerWriteLock(ledgerPath, privateDirectory) {
  await ensureLedgerDirectory(ledgerPath, privateDirectory);
...
async function createLedgerRollbackBackup(database) {
  const directory = await mkdtemp(
    resolve(tmpdir(), "token-ledger-rollback-"),
  );
...
async function restoreLedgerRollbackBackup(backupPath, ledgerPath) {
  const restorePath = `${ledgerPath}.restore-${process.pid}-${Date.now()}`;
  ...
  try {
    await rename(restorePath, ledgerPath);
  } catch (error) {
...
function bindCodexHome(database, codexHome) {
  const selected = canonicalCodexHome(codexHome);
  const stored = metaValue(database, "codex_home");
  if (stored === null) {
    setMeta(database, "codex_home", selected);
    return;
  }
  if (canonicalCodexHome(stored) === selected) {
    if (stored !== selected) setMeta(database, "codex_home", selected);
    return;
  }
...
function reconcileSourceIdentities(database, codexHome, inventory) {
  ...
  for (const row of priorSources) {
    ...
    candidates.push(String(row.sourceId));
  ...
  for (const entry of files) {
    ...
    const candidates = priorByPath.get(
      sourcePathFingerprint(codexHome, entry.path),
    ) || [];
    const priorId = candidates.find((candidate) =>
...
function migrateLegacySnapshot(database, snapshot, now) {
  ...
  const migrationScope = snapshotCollectionScope(snapshot);
  const includeArchived = migrationScope?.includeArchived !== false;
...
function updateSourceStates(database, codexHome, inventory, includeArchived, now) {
  ...
  for (const source of current.values()) {
    ...
    source.reconcileMemberships = Boolean(replaced && !truncated);
    insertSource.run(
      source.sourceId,
      source.sourceLabel,
...
function upsertUsageObservation(
  ...
  const splitCompacted = splitCompactedObservationForEvent(
    database,
    eventKey,
    row,
    metadata,
  );
  if (splitCompacted) positionedObservationId = null;
...
function migrateAndCaptureSources({
    ...
    inventory = {
      ...
      files: (inventory?.files || []).map((entry) => {
        const sourceId = entry.sourceId ||
          sourceIdForPath(codexHome, entry.path);
        return {
          ...entry,
  ...
  const deleteUsageSource = database.prepare(`
    DELETE FROM usage_sources
     WHERE observation_id = ? AND source_id = ?
  `);
  const usageForSource = database.prepare(`
    SELECT observation_id AS observationId
      FROM usage_sources
     WHERE source_id = ?
  `);
  for (const source of sourceMap.values()) {
    ...
    const seen = seenUsageBySource.get(String(source.sourceId)) || new Set();
    for (const row of usageForSource.all(source.sourceId)) {
      if (seen.has(String(row.observationId))) continue;
      deleteUsageSource.run(row.observationId, source.sourceId);
    }
  }
  for (const [eventKey, sourceIds] of eventSources || []) {
    const observationId = exactObservationIds.get(String(eventKey)) ||
  ...
  for (const [key, sourceIds] of quotaSources || []) {
    ...
    for (const sourceId of sourceIds) {
      ...
      `).run(observationId, sourceId, firstSeenAt, lastSeenAt);
    }
  }
...
function databaseUsageRows(database, includeArchived, sources) {
  const activeOnlyMigration = Number(database.prepare(`
    SELECT include_archived AS includeArchived
      FROM migration_runs
     ORDER BY migrated_at DESC
     LIMIT 1
  `).get()?.includeArchived) === 0;
...
function databaseQuotaRows(database, includeArchived, sources) {
  ...
  `).all()
    .map((row) => {
      ...
      if (!includeArchived && Number(row.migrated) !== 1) {
        const scopedBounds = (
          sourceBounds.get(String(row.observationId)) || []
        ).filter((bound) => sources.get(bound.sourceId)?.location === "active");
...
export async function readDurableLedger(
  ...
  try {
    ...
    const migration = migrationRow
      ? {
          ...migrationRow,
          includeArchived: Number(migrationRow.includeArchived) === 1,
        }
      : null;
...
export async function updateDurableLedger({
  ...
  try {
    writeLock = await acquireLedgerWriteLock(ledgerPath, privateDirectory);
    ...
    database = await openLedger(ledgerPath, false, { privateDirectory });
    if (validateAfterCommit instanceof Function) {
      rollbackBackup = await createLedgerRollbackBackup(database);
    }
    ...
    await restrictLedgerFiles(ledgerPath);
    return {
      ...(await readDurableLedger(ledgerPath, {
        includeArchived,
        writerLockHeld: true,
      })),
      committed,
    ...
    if (committed && rollbackBackup) {
      closeDatabase(database);
      database = null;
      try {
        await restoreLedgerRollbackBackup(rollbackBackup.path, ledgerPath);
        committed = false;
      } catch (restoreError) {
    ...
    try {
      await restrictLedgerFiles(ledgerPath);
      if (rollbackBackup) {
        await rm(rollbackBackup.directory, { recursive: true, force: true });
      }
    } finally {
