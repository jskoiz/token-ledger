...
const QUOTA_IDENTITY_CONTRACT_VERSION = "codex-limit-id-v1";
...
function ensureQuotaIdentityContract(database) {
  ...
  // Preview builds keyed missing provider ids by a label or `anonymous`. The
  // opaque hashes cannot be safely rewritten, so make each currently owned
  // quota set reconcile against the next complete scan instead.
  database.prepare(`
    UPDATE source_state
...
function assertPersistedQuotaRows(database) {
  ...
  for (const row of rows) {
    if (!normalizeQuotaObservationFields(row)) {
...
function validateLegacySnapshot(snapshot) {
  ...
  const quotaObservations = legacyOptionalArray(snapshot, "quotaObservations");
  for (const [index, quota] of quotaObservations.entries()) {
...
function migrateLegacySnapshot(database, snapshot, now, codexHome) {
  ...
  const quotaRows = legacyOptionalArray(snapshot, "quotaObservations")
    .map((quota) => {
...
function coalesceStoredQuotaLabels(database, limitKeys) {
  const latestExplicitLabel = database.prepare(`
    SELECT limit_name AS limitName
      FROM quota_observations
     WHERE limit_key = ?
       AND limit_name IS NOT NULL
       AND LENGTH(TRIM(limit_name)) > 0
     ORDER BY last_seen_at DESC, observation_id DESC
     LIMIT 1
  `);
  const applyLabel = database.prepare(`
    UPDATE quota_observations
       SET limit_name = ?
     WHERE limit_key = ?
       AND (limit_name IS NULL OR limit_name <> ?)
  `);
  for (const limitKey of limitKeys) {
    const limitName = latestExplicitLabel.get(limitKey)?.limitName;
    if (limitName == null) continue;
    applyLabel.run(limitName, limitKey, limitName);
  }
}
...
function migrateAndCaptureSources({
  ...
  const affectedQuotaLimitKeys = new Set();
  for (const row of quotas || []) {
    ...
    if (stored) {
      ...
      affectedQuotaLimitKeys.add(stored.limitKey);
    }
  }
  coalesceStoredQuotaLabels(database, affectedQuotaLimitKeys);
...
export async function updateDurableLedger({
  ...
  try {
    ...
    // If that check detects a race, keep this internally complete revision,
    // discard the cache candidate below, and let the collector converge with
    // another complete transaction instead of copying and restoring the
    // entire ledger.
    database.exec("COMMIT");
    committed = true;
    await invokeLedgerFault(
