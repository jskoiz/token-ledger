...
// A rejected post-commit source validation leaves an already-committed
// revision behind. The rejected scan's additions are identifiable because
// association rows it created first carry that run's commit stamp, while
// re-observed rows keep their original first_seen_at. Removing exactly those
// additions lets the retry converge even when it classifies the source as
// truncated, which intentionally never reconciles memberships. Compacted
// associations keep their members' original stamps and legacy-migrated rows
// own no associations, so neither is touched.
function unwindRejectedDurableCommit(database, stamp) {
  database.prepare(
    "DELETE FROM source_event_positions WHERE first_seen_at = ?",
  ).run(stamp);
  database.prepare(`
  ...
  `).run(stamp);
  database.prepare(
    "DELETE FROM tool_sources WHERE first_seen_at = ?",
  ).run(stamp);
  pruneOrphanedReplacedObservations(database);
  database.prepare(`
    UPDATE source_state
       SET observed_event_count = (
         SELECT COUNT(*) FROM usage_sources
          WHERE usage_sources.source_id = source_state.source_id
       )
     WHERE last_seen_at = ?
  `).run(stamp);
}
...
export async function readDurableSourceContinuity(
  ...
  if (sourceStat === null) return new Map();
  let readLock;
  let database;
  try {
    readLock = await acquireLedgerReadLock(ledgerPath);
...
function discardRejectedDurableCommit(database, stamp) {
  try {
    database.exec("BEGIN IMMEDIATE");
    unwindRejectedDurableCommit(database, stamp);
    database.prepare(
    ...
    // The durable pending marker keeps the rejected additions discoverable;
    // the next writer unwinds them before it ingests.
  }
}
...
export async function updateDurableLedger({
  ...
  try {
    ...
    const rejectedStamp = metaValue(
      transactionStatements,
      POST_COMMIT_VALIDATION_PENDING_KEY,
    );
    if (rejectedStamp !== null) {
      // A prior writer committed and then crashed or failed before its
      // post-commit source validation confirmed the scan. Its additions are
      // unvalidated; remove them before this ingest so events still present
      // are re-observed from the current complete scan while transient ones
      // do not survive.
      unwindRejectedDurableCommit(transactionStatements, String(rejectedStamp));
      transactionStatements.prepare(
        "DELETE FROM ledger_meta WHERE key = ?",
      ).run(POST_COMMIT_VALIDATION_PENDING_KEY);
    }
    ...
    if (validateAfterCommit instanceof Function) {
      ...
      // check can confirm the scan. Persist the commit stamp so a rejection,
      // or a crash before the check completes, leaves a durable marker that
      // unwinds the unvalidated additions instead of letting them accrete.
      setMeta(transactionStatements, POST_COMMIT_VALIDATION_PENDING_KEY, now);
    }
    if (metaValue(transactionStatements, "legacy_snapshot_checked") !== "1") {
    ...
    // unwind the rejected scan's own additions, so the collector converges
    // even when the racing change was a truncation, which never reconciles
    // memberships.
    database.exec("COMMIT");
    committed = true;
    await invokeLedgerFault(
    ...
    if (validateAfterCommit instanceof Function) {
      try {
        ...
        discardRejectedDurableCommit(database, now);
