...
async function acquireLedgerRollbackSnapshot(ledgerPath) {
  let database;
  try {
    // Keep this connection writable so a failed attempt can copy the pinned
    // read view into an attached replacement database. The main ledger is
    // only read through this connection.
    database = new DatabaseSync(ledgerPath);
    database.exec(`PRAGMA busy_timeout = ${LEDGER_BUSY_TIMEOUT_MS}`);
    database.exec("BEGIN");
    // Establish the WAL read view before the candidate transaction starts.
    database.prepare("SELECT COUNT(*) AS count FROM ledger_meta").get();
    const tables = LEDGER_TABLES.map((name) => {
      const table = quotedSqlIdentifier(name);
      const columns = database.prepare(
        `PRAGMA main.table_info(${table})`,
      ).all().map((column) => quotedSqlIdentifier(column.name));
      if (!columns.length) {
        throw new Error(`Cannot snapshot durable-ledger table ${name}.`);
      }
...
function releaseLedgerRollbackSnapshot(snapshot) {
  ...
  try {
    ...
    // Closing the connection also releases its pinned WAL read view.
  }
  closeDatabase(database);
}

async function restoreLedgerRollbackSnapshot(snapshot, ledgerPath) {
  const snapshotDatabase = snapshot?.database;
  if (!snapshotDatabase) {
    throw new Error("The durable-ledger rollback snapshot is unavailable.");
  }
  const directory = await mkdtemp(
    resolve(dirname(ledgerPath), ".token-ledger-rollback-"),
  );
  const restorePath = resolve(directory, "ledger.sqlite");
  await chmodIfPresent(directory, 0o700);
  let restoreDatabase;
  let restored = false;
  try {
    // Materialize the pinned pre-write view only when post-commit validation
    // fails. ATTACH and ordinary SQL keep this compatible with the minimum
    // Node release that provides node:sqlite; successful refreshes pay only
    // for the read transaction above.
    snapshotDatabase.prepare(
      "ATTACH DATABASE ? AS rollback_snapshot",
    ).run(restorePath);
    for (const { table, columns } of snapshot.tables) {
      const columnList = columns.join(", ");
      snapshotDatabase.exec(`
        CREATE TABLE rollback_snapshot.${table} AS
        SELECT ${columnList} FROM main.${table};
      `);
    }
    snapshotDatabase.exec("COMMIT");
    snapshot.database = null;
    closeDatabase(snapshotDatabase);
    await chmodIfPresent(restorePath, 0o600);

    restoreDatabase = new DatabaseSync(ledgerPath);
    restoreDatabase.exec(`
      PRAGMA busy_timeout = ${LEDGER_BUSY_TIMEOUT_MS};
      PRAGMA foreign_keys = ON;
    `);
    restoreDatabase.prepare(
      "ATTACH DATABASE ? AS rollback_snapshot",
    ).run(restorePath);
    restoreDatabase.exec("BEGIN IMMEDIATE");
    restoreDatabase.exec("PRAGMA defer_foreign_keys = ON");
    for (const { table } of [...snapshot.tables].reverse()) {
      restoreDatabase.exec(`DELETE FROM main.${table}`);
    }
    for (const { table, columns } of snapshot.tables) {
      const columnList = columns.join(", ");
      restoreDatabase.exec(`
        INSERT INTO main.${table} (${columnList})
        SELECT ${columnList} FROM rollback_snapshot.${table};
      `);
    }
    const restoredRevision = Number(
      metaValue(restoreDatabase, "revision") || 0,
    );
    if (restoredRevision !== snapshot.revision) {
      throw new Error("The durable-ledger rollback revision did not match.");
    }
    restoreDatabase.exec("COMMIT");
    restored = true;
    await restrictLedgerFiles(ledgerPath);
  } finally {
    if (!restored) {
      try {
        restoreDatabase?.exec("ROLLBACK");
      } catch {
        // The restoration error remains authoritative.
      }
    }
    closeDatabase(restoreDatabase);
    releaseLedgerRollbackSnapshot(snapshot);
...
function updateSourceStates(
  ...
  for (const source of current.values()) {
    ...
    const truncated = prior && !fileIdentityChanged && source.size < previousSize;
    ...
    // Unseen members are meaningful only after a complete source scan.
    const sourceMembershipsComplete = !uncertainIds.has(
      String(source.sourceId),
    );
    source.reconcileMemberships = Boolean(
      sourceWasIngested &&
      sourceMembershipsComplete &&
      !truncated &&
      !relocated &&
      replaced,
    );
...
export async function readDurableLedger(
  ...
  let database;
  try {
    if (!writerLockHeld) readLock = await acquireLedgerReadLock(ledgerPath);
...
export async function readDurableSourceContinuity(path) {
  const ledgerPath = resolve(path);
  try {
...
export async function updateDurableLedger({
  ...
  try {
    ...
    if (validateAfterCommit instanceof Function) {
      rollbackSnapshot = await acquireLedgerRollbackSnapshot(ledgerPath);
    }
    database.exec("BEGIN IMMEDIATE");
    ...
    if (faultInjector instanceof Function) {
      await faultInjector({ point: "before-commit", path: ledgerPath });
    }
    if (validateBeforeCommit instanceof Function) {
      await validateBeforeCommit({ path: ledgerPath });
    }
    if (faultInjector instanceof Function) {
      await faultInjector({ point: "after-validation", path: ledgerPath });
    }
    ...
    committed = true;
    if (faultInjector instanceof Function) {
      await faultInjector({ point: "after-sqlite-commit", path: ledgerPath });
    }
    if (validateAfterCommit instanceof Function) {
      await validateAfterCommit({ path: ledgerPath });
    }
    releaseLedgerRollbackSnapshot(rollbackSnapshot);
    rollbackSnapshot = null;
    if (stagedCommit) await stagedCommit.publish();
    if (faultInjector instanceof Function) {
      await faultInjector({ point: "after-commit", path: ledgerPath });
    }
    await restrictLedgerFiles(ledgerPath);
    return {
      ...committedLedger,
    ...
    if (committed && rollbackSnapshot) {
      ...
      database = null;
      try {
        await restoreLedgerRollbackSnapshot(rollbackSnapshot, ledgerPath);
        rollbackSnapshot = null;
        committed = false;
      } catch (restoreError) {
