...
function observationForSourcePosition(database, positions, eventKey) {
  ...
  for (const position of positions || []) {
    ...
    return {
      observationId: String(positioned.observationId),
      replacesEventKey: positioned.eventKey == null
        ? null
        : String(positioned.eventKey),
    };
  }
  return null;
}
...
function databaseUsageRows(database, includeArchived, sources) {
  ...
  `).all()) {
    ...
    if (row.identityKind === "exact" || row.identityKind === "compacted") {
      ...
      observedCandidates.push(usageRow);
      if (!sourceAssociationAllows(associated, sources, includeArchived)) {
        continue;
      }
    }
    rows.push(usageRow);
    ...
    SELECT call_key AS callKey, turn_id AS turnId, thread_id AS threadId,
           ...
           EXISTS (
             SELECT 1 FROM usage_tool_membership AS membership
              WHERE membership.call_key = tool_observations.call_key
           ) AS usageOwned
      FROM tool_observations
  `).all()
    ...
    .map((row) => ({
      ...
      threadId: text(row.threadId, 400),
      originalLikely: Number(row.originalLikely) === 1,
      usageOwned: Number(row.usageOwned) === 1,
