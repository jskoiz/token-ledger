...
// Every table the ingest pass may mutate. The commit undo log captures those
// mutations so a revision rejected by post-commit source validation can be
// reverted exactly instead of being approximated from timestamps.
...
// Re-observing existing rows bumps only these columns; skipping them keeps
// the undo log proportional to real changes instead of scan volume.
...
// Row-level logical undo for the ingest pass. Temporary triggers capture, in
// commit order, the prior image of every updated or deleted row and the key
// of every inserted row across the ingest-mutable tables. The log commits
...
// reconciliation deletions, and net-new additions alike. Compaction and
// retention pruning run after the capture window because they are
// content-preserving transforms that no rejection invalidates.
function createCommitUndoCapture(database) {
  ...
  for (const table of COMMIT_UNDO_TABLES) {
    ...
    const changed = columns
      .filter((column) => !COMMIT_UNDO_VOLATILE_COLUMNS.has(column.name))
...
export async function updateDurableLedger({
  ...
  try {
    ...
    if (
      ...
      // post-commit source validation confirmed the scan. Revert that
      // unvalidated revision from its undo log before this ingest so events
      // still present are re-observed from the current complete scan while
      // transient ones do not survive.
      applyCommitUndoLog(transactionStatements);
