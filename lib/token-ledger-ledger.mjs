...
async function validateLedgerMainPath(path, { allowMissing = false } = {}) {
  let fileStat;
  try {
  ...
  if (fileStat.isSymbolicLink()) {
    throw ledgerPathError(
      "The durable ledger path must not be a symbolic link.",
    );
  }
  if (!fileStat.isFile()) {
    throw ledgerPathError(
      "The durable ledger path must be a regular file.",
    );
  }
  if (Number(fileStat.nlink) !== 1) {
    throw ledgerPathError(
      "The durable ledger path must have exactly one filesystem link.",
    );
  }
  return fileStat;
}
...
async function openLedger(
  ...
  const beforeOpen = await validateLedgerMainPath(path, {
    allowMissing: !readOnly,
  });
  const database = new DatabaseSync(path, readOnly ? { readOnly: true } : {});
  try {
    const afterOpen = await validateLedgerMainPath(path);
    if (beforeOpen !== null && !sameFileIdentity(beforeOpen, afterOpen)) {
      throw ledgerPathError(
        "The durable ledger path changed while it was being opened.",
      );
    }
  } catch (error) {
...
async function prepareLedgerRecovery(
  ...
  await validateLedgerMainPath(ledgerPath);
  const paths = ledgerRecoveryPaths(ledgerPath);
...
async function restoreLedgerRecovery(
  ...
  await validateLedgerMainPath(recovery.ledgerPath, { allowMissing: true });
...
function validateLegacyQuota(quota, index) {
  ...
  const resetsAt = primitiveNumber(quota.resetsAt);
  const usedPercent = primitiveNumber(quota.usedPercent);
  if (
    !Number.isSafeInteger(quota.windowMinutes) ||
