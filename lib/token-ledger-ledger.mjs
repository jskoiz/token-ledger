...
import { safeExportLabel } from "./token-ledger-labels.mjs";
...
const TEST_STATE_NAMESPACE = /^\d+$/.test(
  ...
  ? process.env.TOKEN_LEDGER_TEST_STATE_NAMESPACE
  : String(process.pid);
const TEST_PRIVATE_STATE_BASE = process.env.TOKEN_LEDGER_TEST_STATE_ROOT
  ? resolve(process.env.TOKEN_LEDGER_TEST_STATE_ROOT)
  : resolve(DEFAULT_PRIVATE_STATE_DIRECTORY, "test-state");
const TEST_PRIVATE_STATE_ROOT = resolve(
  TEST_PRIVATE_STATE_BASE,
  TEST_STATE_NAMESPACE,
);
const QUOTA_IDENTITY_CONTRACT_KEY = "quota_identity_contract";
const THREAD_METADATA_FINGERPRINTS_KEY = "thread_metadata_fingerprints";
...
const LEDGER_SCHEMA_MIGRATION_BATCH_SIZE = 512;
...
function serializedJsonObject(value) {
  const object = parsedJsonObject(value);
...
function hasTimestampCorrection(value) {
  const correction = parsedJsonObject(value)?.timestampCorrection;
  return Boolean(
    correction &&
      typeof correction === "object" &&
      !Array.isArray(correction) &&
      correction.kind === TIMESTAMP_CORRECTION_KIND &&
      primitiveString(correction.originalTimestamp) !== null &&
      primitiveString(correction.fallbackTimestamp) !== null,
  );
}

function threadMetadataFingerprintMap(value) {
  const parsed = parseJson(value);
  if (!Array.isArray(parsed)) return null;
  const fingerprints = new Map();
  for (const entry of parsed) {
    if (!Array.isArray(entry) || entry.length !== 2) return null;
    const threadId = primitiveString(entry[0]);
    const fingerprint = primitiveString(entry[1]);
    if (
      threadId === null ||
      threadId.length === 0 ||
      fingerprint === null ||
    ...
    fingerprints.set(threadId, fingerprint.toLowerCase());
  }
  return fingerprints;
}

function serializeThreadMetadataFingerprints(fingerprints) {
  if (!(fingerprints instanceof Map)) return null;
  const entries = [...fingerprints]
    .map(([threadId, fingerprint]) => [
      String(threadId),
      String(fingerprint).toLowerCase(),
    ])
    .sort(([left], [right]) => left.localeCompare(right));
  if (entries.some(([, fingerprint]) => !/^[0-9a-f]{64}$/.test(fingerprint))) {
    throw new Error("Invalid thread metadata fingerprint.");
  }
...
function migrateLedgerSchemaV2ToV3(database) {
  ...
  try {
    database.exec(ledgerSchema());
    const firstPositions = database.prepare(`
      SELECT source_id AS sourceId, event_ordinal AS eventOrdinal,
             event_key AS eventKey
        FROM source_event_positions
       ORDER BY source_id, event_ordinal
       LIMIT ?
    `);
    const nextPositions = database.prepare(`
      SELECT source_id AS sourceId, event_ordinal AS eventOrdinal,
             event_key AS eventKey
        FROM source_event_positions
       WHERE source_id > ?
          OR (source_id = ? AND event_ordinal > ?)
       ORDER BY source_id, event_ordinal
       LIMIT ?
    `);
    ...
    `);
    let positions = firstPositions.all(LEDGER_SCHEMA_MIGRATION_BATCH_SIZE);
    while (positions.length > 0) {
      ...
      if (positions.length < LEDGER_SCHEMA_MIGRATION_BATCH_SIZE) break;
      const last = positions.at(-1);
      positions = nextPositions.all(
        String(last.sourceId),
        String(last.sourceId),
        Math.trunc(Number(last.eventOrdinal)),
        LEDGER_SCHEMA_MIGRATION_BATCH_SIZE,
      );
    }
...
function usageRowFromDatabase(row, sourceIds = new Set()) {
  ...
  const result = {
    ...
    project: safeExportLabel(row.project, 160, "Unknown project"),
...
function subtractMigratedContribution(state, candidate) {
  ...
  state.residual.rangeAllocationOrigin = state.row.rangeAllocationOrigin || {
...
function migrationRowFromBucket(bucket, migrationKey, index, generatedAt, now) {
  ...
  return {
    ...
    project: safeExportLabel(normalized.project, 160, "Unknown project"),
...
function migrateLegacySnapshot(database, snapshot, now, codexHome) {
  ...
  for (const thread of legacyOptionalArray(snapshot, "threads")) {
    ...
    insertThread.run(
      ...
      safeExportLabel(thread.project, 160, "Unknown project"),
...
function upsertUsageObservation(
  ...
  const rangeAllocationEstimated = Number(row.rangeAllocationEstimated) === 1
    ? 1
    : 0;
  const timestampRepair = rangeAllocationEstimated === 1 &&
    hasTimestampCorrection(row.rangeAllocationOrigin);
  const rowThreadId = text(row.threadId, 400);
  const rowTurnId = text(row.turnId, 400);
  const rowOriginThreadId = text(row.originThreadId, 400);
  const timestampMs = finiteTimestamp(row.timestamp);
  const originTimestampMs = finiteTimestamp(row.originTimestamp);
  const timestampEvidenceIsActual = !timestampRepair &&
    rangeAllocationEstimated === 0 &&
    row.rangeAllocationOrigin == null &&
    rowTurnId.length > 0 &&
    rowThreadId.length > 0 &&
    rowOriginThreadId === rowThreadId &&
    timestampMs !== null &&
    originTimestampMs !== null &&
    timestampMs >= originTimestampMs;
  const timestampEvidenceIsUnscoped = !timestampRepair &&
    rangeAllocationEstimated === 0 &&
    row.rangeAllocationOrigin == null &&
    rowOriginThreadId.length === 0;
  const rangeAllocationOrigin = serializedJsonObject(
  ...
  // Better event-time evidence may replace this repair's fallback, but it
  // must not bypass the original/copy precedence for otherwise valid rows.
  const promoteActualTimestamp = `(
    (SELECT value FROM timestamp_actual_flag) = 1
    AND usage_observations.range_allocation_estimated = 1
    AND json_extract(
      usage_observations.range_allocation_origin,
      '$.timestampCorrection.kind'
    ) = 'before-own-turn'
  )`;
  const insert = database.prepare(`
    WITH replacement_flag(value) AS (VALUES (?)),
         timestamp_repair_flag(value) AS (VALUES (?)),
         timestamp_actual_flag(value) AS (VALUES (?)),
         timestamp_unscoped_flag(value) AS (VALUES (?))
    INSERT OR IGNORE INTO usage_observations (
      observation_id, identity_kind, event_key, turn_id, thread_id, timestamp,
      input_tokens, cached_input_tokens, cache_write_input_tokens,
      output_tokens, reasoning_tokens, total_tokens, tool_calls, call_count,
    ...
    ON CONFLICT(observation_id) DO UPDATE SET
      ...
      turn_id = CASE WHEN (SELECT value FROM replacement_flag) = 1
          ...
          OR (SELECT value FROM timestamp_repair_flag) = 1
          OR ${promoteActualTimestamp}
      ...
      timestamp = CASE WHEN (SELECT value FROM replacement_flag) = 1
          OR (
            usage_observations.original_likely = 0
            AND (
              (SELECT value FROM timestamp_actual_flag) = 1
              OR (SELECT value FROM timestamp_unscoped_flag) = 1
              OR (
                (SELECT value FROM timestamp_repair_flag) = 1
                AND (
                  usage_observations.range_allocation_estimated = 1
                  OR julianday(usage_observations.timestamp) <
                    julianday(excluded.origin_timestamp)
                )
              )
            )
          )
          OR (
            excluded.original_likely = 1
            AND (
              (SELECT value FROM timestamp_repair_flag) = 0
              OR usage_observations.range_allocation_estimated = 1
              OR julianday(usage_observations.timestamp) <
                julianday(excluded.origin_timestamp)
            )
          )
      ...
      range_allocation_estimated = CASE WHEN (SELECT value FROM replacement_flag) = 1
        ...
        WHEN (SELECT value FROM timestamp_repair_flag) = 1
          AND (
            usage_observations.range_allocation_estimated = 1
            OR julianday(usage_observations.timestamp) <
              julianday(excluded.origin_timestamp)
          ...
          THEN excluded.range_allocation_estimated
        WHEN excluded.original_likely = 1
          AND (SELECT value FROM timestamp_repair_flag) = 0
          THEN excluded.range_allocation_estimated
        WHEN ${promoteActualTimestamp}
          THEN excluded.range_allocation_estimated
      ...
      range_allocation_origin = CASE WHEN (SELECT value FROM replacement_flag) = 1
        ...
        WHEN excluded.original_likely = 1
          AND (SELECT value FROM timestamp_repair_flag) = 0
          THEN CASE WHEN json_extract(
              usage_observations.range_allocation_origin,
              '$.timestampCorrection.kind'
            ) = 'before-own-turn'
            THEN usage_observations.range_allocation_origin
            ELSE excluded.range_allocation_origin END
        WHEN ${promoteActualTimestamp}
          THEN CASE WHEN json_extract(
              usage_observations.range_allocation_origin,
              '$.timestampCorrection.kind'
  ...
  insert.run(
    replaceIdentity ? 1 : 0,
    timestampRepair ? 1 : 0,
    timestampEvidenceIsActual ? 1 : 0,
    timestampEvidenceIsUnscoped ? 1 : 0,
    observationId,
    "exact",
    eventKey,
    text(row.turnId, 400),
    ...
    safeExportLabel(metadataRecord.project, 160, "Unknown project"),
...
function upsertThreadRecord(database, row, now) {
  ...
  `).run(
    ...
    safeExportLabel(row.project, 160, "Unknown project"),
...
function repairPersistedUsageTimestamps(database) {
  const candidates = database.prepare(`
    SELECT observation_id AS observationId,
     ...
     WHERE identity_kind = 'exact'
       ...
       AND origin_timestamp IS NOT NULL
       AND julianday(timestamp) < julianday(origin_timestamp)
  `).all();
  if (!candidates.length) return 0;
  const repair = database.prepare(`
    UPDATE usage_observations
       SET timestamp = ?,
           range_allocation_estimated = 1,
           range_allocation_origin = ?
     WHERE observation_id = ? AND identity_kind = 'exact'
  `);
  let repaired = 0;
  for (const row of candidates) {
    const timestampMs = finiteTimestamp(row.timestamp);
    const originTimestamp = safeIso(row.originTimestamp, null);
    const originMs = finiteTimestamp(originTimestamp);
    if (
      timestampMs === null ||
      originMs === null ||
      timestampMs >= originMs
    ) continue;
    const timestamp = safeIso(row.timestamp, null);
    if (!timestamp) continue;
    const origin = timestampCorrectionOrigin(
      row.rangeAllocationOrigin,
      timestamp,
      originTimestamp,
    );
...
function repairImpossibleDetailedCallCounts(database) {
  // A positive exact total with no input or output tokens has no detailed
  // breakdown, even when an older importer incorrectly marked components as
  // valid. Compacted rows are deliberately excluded because their partial
  // coverage can legitimately produce a zero component slice.
  return database.prepare(`
    UPDATE usage_observations
       SET detailed_call_count = 0
     WHERE identity_kind = 'exact'
       AND total_tokens > 0
       AND input_tokens = 0
       AND output_tokens = 0
       AND detailed_call_count > 0
  `).run().changes;
}
...
function migrateAndCaptureSources({
  ...
  // These deterministic repairs run inside the same transaction as source
  // reconciliation, so cache-reused sources converge just like rescanned
  // sources and the commit undo log can revert them together.
  repairPersistedUsageTimestamps(database);
  repairImpossibleDetailedCallCounts(database);
  ...
  const serializedThreadMetadataFingerprints =
    serializeThreadMetadataFingerprints(threadMetadataFingerprints);
  if (
    serializedThreadMetadataFingerprints !== null &&
    metaValue(database, THREAD_METADATA_FINGERPRINTS_KEY) !==
      serializedThreadMetadataFingerprints
  ) {
    setMeta(
      database,
      THREAD_METADATA_FINGERPRINTS_KEY,
      serializedThreadMetadataFingerprints,
    );
  }
...
function compactedAggregate(existing, bucket, nowMs) {
  ...
  return {
    ...
    project: safeExportLabel(bucket.project, 160, "Unknown project"),
...
function materializeDurableLedger(
  ...
  if (
    version !== DURABLE_LEDGER_SCHEMA_VERSION_V2 &&
    version !== DURABLE_LEDGER_SCHEMA_VERSION
  ) {
    const error = new Error("Unsupported Token Ledger durable ledger schema.");
    error.code = "ERR_DURABLE_LEDGER_SCHEMA";
    throw error;
...
export async function readDurableSourceContinuity(
  ...
  if (sourceStat === null) {
    return {
      ...
      threadMetadataFingerprints: null,
      threadRecords: null,
    };
  }
  let readLock;
  ...
  try {
    ...
    const threadMetadataFingerprints = threadMetadataFingerprintMap(
      metaValue(database, THREAD_METADATA_FINGERPRINTS_KEY),
    );
    return {
      sources: continuity,
      ...
      threadRecords: new Map(
        databaseThreadRows(database).map((row) => [String(row.id), row]),
      ),
    };
  } catch (error) {
    const sqliteErrcode = Number(error?.errcode);
...
export async function updateDurableLedger({
  ...
  try {
    ...
    try {
      ...
      // The ledger and snapshot are already published. Storage reclamation is
      // advisory and can retry on a later refresh; it must not turn a
      // successful durable commit into a reported failure.
    }
